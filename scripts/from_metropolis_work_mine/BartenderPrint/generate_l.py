...
class Contents:
    ...
    @staticmethod
    def cyrillic_support(babel="english,russian", pdflatex_font="\sfdefault"):
        string = r"\usepackage{ifxetex}  % For work both with pdflatex and xelatex" + newline
        string += "\ifxetex" + newline
        string += "    %% xelatex" + newline
        string += r"    \usepackage{pst-barcode}  % barcodes enable" + newline
        # string += r"    \usepackage[tightpage,active]{preview}" + newline
        # string += r"    \PreviewBorder=0pt" + newline
        # string += r"    \PreviewEnvironment{pspicture}" + newline
        ...
        string += "    \setmainfont[Ligatures={TeX}]{Arial}  % Default font of document" + newline
        string += "    \setsansfont{Arial}  % Font without serifs" + newline
        string += "    \setmonofont{Arial}  % Mono font" + newline
        ...
        string += r"    \renewcommand{\familydefault}{" + pdflatex_font + "}  % change font" + newline
        #######################################test###################################################
        # string += r"    \usepackage{mathptmx}" + newline + r"    \usepackage{anyfontsize}" + newline + r"    \usepackage{t1enc}" + newline
        #######################################test###################################################
        string += r"\fi" + newline
    ...
    def to_inches(value_in_mm):
        in_inches = str(int(value_in_mm) / 25.39)  # 25.3 give "Overfull \hbox (0.60028pt too wide) in paragraph at lines __--__" error#################################################
    ...
    @staticmethod
    def textbox(textvar, type, huge=False):  # type can be   centered   or s t r e t c h e d
        string = r"\makebox[\textwidth][" + type[:1] + r"]{\textbf{" + backslash + textvar + "}}  % box with text" + newline
        if huge:
            string = r"\makebox[\textwidth][" + type[:1] + r"]{{\fontsize{28}{30} \selectfont \textbf{" + backslash + textvar + "}}}  % box with big text" + newline
        return string
...
class Page():
    ...
    def createpage_58_60(bar_text, second_line=None, barcode_type="code128"):
        class CurrentPage():
            width = 52
            height = 55
            height_of_5860_text = 9  # mm     not enough 8    10 no problem  # 6mm ok for 14pt font
        ...
        Codegen.add_line(Contents.begin("pspicture", "[showgrid=false](" + str(CurrentPage.width) + "mm," +
                                        str(CurrentPage.free_height) +
                                        "mm)%(0,0)(10,10) % idk what is that, size doesn't changing"))
        ...
        Codegen.add_line(Contents.no_indent())
        #################################################################################test
        # Codegen.add_line(r"{\fontsize{50}{60}\selectfont Foo}" + newline)
        #################################################################################test
        Codegen.add_line(Contents.textbox("bartext", "centered", huge=True))
        if second_line:
            cnt = 0
            for line in CurrentPage.text_lines:
                cnt += 1
...
if State.legacy_print:
    ...
    while cnt < count:
        ...
        # Page.createpage_58_60("TR05", "конечная 5 группа")
        # Page.createpage_58_60("TR05", "конечная 5 группа")
        Page.createpage_58_60("TR07-01", "7 группа транзитная")
