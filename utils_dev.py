...
def file_create(filename):
    filename = path_full(filename)
    dir_create(os.path.split(filename)[0])  # todo change it to just dir_create(filename)
...
def file_hide(filename, quiet=True):
    ...
    dotted_file = path_extend(dotted_file[0], "." + dotted_file[1])  # merging it back and add dot
    debug_print("filename", filename, "dotted_file", dotted_file, "")
    plog("def_file_hide.tst", "filename " + filename + " dotted_file "+ dotted_file, backup=False)
...
def stringWidthNewlinesToString(string):
    if get_os() == "windows":
...
def getOutPutFromCommand(command, quiet = True, split_lines=False):
    p = subprocess.check_output(command, shell=True)
    output = p.decode("cp866")
    if split_lines:
        output = stringWidthNewlinesToString(output)
    return output
...
def home_path():
    ...
    homepath = homepath.rstrip(newline2) # todo взять на заметку в getOutputFromCommand & stringWidthNewlinesToString
    return homepath
