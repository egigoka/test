    ...
    def calculate(self, *ignore):
        ...
        try:
            ...
            if match:
                ...
                # If there are no match groups, match.lastindex will be
                # None (which is not valid for range()), but
                # match.group(0) is always valid so should always be
                # shown. Of course we can't show more groups than we
                # have labels for.
                limit = min(len(self.groupLabels),
                            1 + (match.lastindex
                                 if match.lastindex is not None else 0))
